...
public class AdminDaoImpl implements IAdminDao {
	...
	public List<ApplicationBean> getApplicantsByStatus(String status)
			throws UniversityException {
		TypedQuery<ApplicationBean> qry = manager.createQuery(IQueryMapper.LIST_APPLICANTS_BYONLYSTATUS,ApplicationBean.class);
		qry.setParameter("st",status);
		List<ApplicationBean> applist = qry.getResultList();
		return applist;
		
	}

	@Override
	public ProgramScheduledBean getScheduledProgListForProgramId(
			String pId) throws UniversityException {
		ProgramScheduledBean schedule = manager.find(ProgramScheduledBean.class, pId);
		return schedule;
	}

	@Override
	public void updateScheduledProg(String pId,ProgramScheduledBean bean) throws UniversityException{
		ProgramScheduledBean updatebean = manager.find(ProgramScheduledBean.class, pId);
		updatebean.setEndDate(bean.getEndDate());
		updatebean.setStartDate(bean.getStartDate());
		updatebean.setSessionPerWeek(bean.getSessionPerWeek());
