...
public class ApplicantDaoImpl implements IApplicantDao {
	...
	@Override
	public List<ProgramScheduledBean> applicantSheduledProgramlistbyLocation(
			String location) throws UniversityException {
		TypedQuery<ProgramScheduledBean> qry = manager.createQuery(IQueryMapper.LIST_PROGRAM_BY_LOC,ProgramScheduledBean.class);
		qry.setParameter("ptitle",location);
		List<ProgramScheduledBean> programSchList = qry.getResultList();
		return programSchList;
	}

	@Override
	public List<String> getEmailId() throws UniversityException {
		TypedQuery<String> qry = manager.createQuery(IQueryMapper.LIST_EMAIL_ID,String.class);
		List<String> emailList = qry.getResultList();
		return emailList;	
	}
}